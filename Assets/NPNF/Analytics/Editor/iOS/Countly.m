...
@property (nonatomic, copy) NSString *skieClientId;
@property (nonatomic, copy) NSString *sessionId;
@property (nonatomic, copy) NSString *skieUserId;
...
- (void) tick
{
    ...
    [request addValue:self.accessToken forHTTPHeaderField:@"access_token"];
...
- (void)beginSession
{
    ...
    NSLog(@"%s: sessionId=%@", __FUNCTION__, self.sessionId);

	NSString *data = [NSString stringWithFormat:@"app_key=%@&skie_client_id=%@&skie_user_id=%@&skie_session_id=%@&timestamp=%ld&sdk_version="COUNTLY_VERSION"&begin_session=1&metrics=%@",
					  self.appKey,
					  self.skieClientId,
						self.skieUserId,
						self.sessionId,
...
{
	NSString *data = [NSString stringWithFormat:@"app_key=%@&skie_client_id=%@&skie_user_id=%@&skie_session_id=%@&timestamp=%ld&session_duration=%d",
					  self.appKey,
					  self.skieClientId,
						self.skieUserId,
						self.sessionId,
...
{
	NSString *data = [NSString stringWithFormat:@"app_key=%@&skie_client_id=%@&skie_user_id=%@&skie_session_id=%@&timestamp=%ld&end_session=1&session_duration=%d",
					  self.appKey,
					  self.skieClientId,
						self.skieUserId,
						self.sessionId,
...
{
	NSString *data = [NSString stringWithFormat:@"app_key=%@&skie_client_id=%@&skie_user_id=%@&skie_session_id=%@&timestamp=%ld&events=%@",
					  self.appKey,
					  self.skieClientId,
						self.skieUserId,
						self.sessionId,
...
- (void)start:(NSString *)appKey withHost:(NSString *)appHost clientId:(NSString *)clientId accessToken:(NSString *)accessToken
{
    ...
    [[CountlyConnectionQueue sharedInstance] setSkieClientId:clientId];
    [[CountlyConnectionQueue sharedInstance] setAccessToken:accessToken];
	[[CountlyConnectionQueue sharedInstance] beginSession];
    
    NSLog(@"Countly: started");
}

- (void)updateAccessToken:(NSString *)value
{
    [[CountlyConnectionQueue sharedInstance] setAccessToken:value];
}

- (void)setUserId:(NSString*)userId
{
    [[CountlyConnectionQueue sharedInstance] setSkieUserId:userId];
}

- (void)startOnCloudWithAppKey:(NSString *)appKey clientId:(NSString *)clientId
{
    [self start:appKey withHost:@"https://cloud.count.ly" clientId:clientId accessToken:nil];
}
