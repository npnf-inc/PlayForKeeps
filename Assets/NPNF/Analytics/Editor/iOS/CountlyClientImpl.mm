...
extern "C"
{
    ...
    void _Start(const char* serverUrl, const char* appKey, const char* clientId, const char* accessToken)
    {
        NSLog(@"Countly: serverURL=%s", serverUrl);
        [[Countly sharedInstance] start:Countly_CreateNSString(appKey) withHost:Countly_CreateNSString(serverUrl) clientId:Countly_CreateNSString(clientId) accessToken:Countly_CreateNSString(accessToken)];
    }
